...
# Caminho da imagem da chancela
chancela_path = r"C:\xampp\htdocs\docmark\chancela\config\chancela.png"
...
# Carregar a imagem da chancela e redimensionar para 180x180
chancela = Image.open(chancela_path).resize((180, 180))
...
for filename in os.listdir(pdf_dir):
    if filename.endswith('.pdf'):
        ...
        try:
            ...
            images_1bit_with_stamp = []
            
            for image in images:
                # Adicionar a chancela no canto superior direito
                largura_pagina, altura_pagina = image.size
                largura_chancela, altura_chancela = chancela.size
                
                # Calcular posição no canto superior direito
                posicao_x = largura_pagina - largura_chancela - 10  # 10px de margem
                posicao_y = 10  # 10px de margem

                # Criar uma cópia da página e adicionar a chancela
                pagina_com_chancela = image.copy()
                pagina_com_chancela.paste(chancela, (posicao_x, posicao_y), chancela)

                # Converter para 1-bit (preto e branco)
                pagina_1bit = pagina_com_chancela.convert('1')
                images_1bit_with_stamp.append(pagina_1bit)

            # Salvar a primeira página como TIFF com compressão Group 4
            images_1bit_with_stamp[0].save(tiff_path, save_all=True, append_images=images_1bit_with_stamp[1:], compression="group4")
