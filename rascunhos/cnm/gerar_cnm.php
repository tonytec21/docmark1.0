...
if (file_exists('cns.json')) {
    ...
    if (isset($cnsData['cns'])) {
        ...
        if (file_exists('numero_matricula.json')) {
            ...
            if (isset($numeroMatriculaData['numeroMatricula'])) {
                ...
                // Salvar o CNM em um arquivo JSON
                $cnmData = array(
                    'cnm' => $cnm
                );

                // Converter o array em formato JSON
                $cnmJson = json_encode($cnmData);

                // Salvar o JSON em um arquivo
                file_put_contents('cnm.json', $cnmJson);

                // Redirecionar para a página de carimbo com o CNM e o nome do arquivo PDF
                header('Location: carimbo.php?cnm=' . urlencode($cnm));
                exit();
            } else {
                echo "Campo 'numeroMatricula' não encontrado no arquivo 'numero_matricula.json'.";
            }
        } else {
