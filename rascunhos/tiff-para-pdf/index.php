...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_FILES['files']['name'])) {
    if (count($_FILES['files']['name']) > 20) {
        echo "Erro: Não é permitido selecionar mais de 20 arquivos por vez.";
        exit;
    }
    ...
    <div class="container">
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_FILES['files']['name'])) {
            ...
            if (file_exists($zip_file)) {
                $zip_download_url = 'http://' . $_SERVER['HTTP_HOST'] . '/docmark/tiff-para-pdf/arquivos/' . basename($zip_file);
